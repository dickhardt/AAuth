...
- **Consumption record**: A `{jti, consumed}` pair reporting what the resource metered against one prior auth token's budget. Carried in the `budget_consumed` claim of a resource token.
...
## Consumption Records {#budget-consumed}
...
The records ride in the resource token because it already travels resource → agent → PS at exactly the moment the PS re-decides: no extra round trip, resource-signed, and interpretable without a metadata fetch. Totals over horizons longer than 20 grants are served at the usage endpoint (#usage-counters).
...
Three cases need more, and the base protocol answers each.

**The person holds several accounts at the resource.** Account Binding (([@!I-D.hardt-oauth-aauth-protocol]), Account Binding) covers this: the authorization endpoint request carries an OPTIONAL `account` parameter naming one of them from the resource's own namespace, the resource echoes it as the `account` claim of the resource token, and it is copied into the auth token. A metered resource that bills per account SHOULD require `account` where a person may hold more than one, because a budget enforced against the wrong account is charged to the wrong payer.

**The person has no account at the resource yet, or one that is not funded.** This is the first budgeted request from a `sub` the resource has not seen, and it is a question for the person rather than the agent. Resource-Initiated Interaction (([@!I-D.hardt-oauth-aauth-protocol]), Resource-Initiated Interaction) is the mechanism: the resource puts an `interaction` claim in the resource token, the person server chains the person through the resource's own flow — signing in, creating an account, connecting a payment method — and completes its own consent afterwards. Because the resource issues the resource token, it decides when to ask again: once per person, or once per mission, since it sees `mission_s256` at that moment.

**Four-party access, where the access server holds the commercial relationship.** Nothing needs to travel in a claim. The AS receives a token request from a person server carrying a `sub` it has not seen, returns `202` with `requirement=interaction` (([@!I-D.hardt-oauth-aauth-protocol]), Access Server Federation), and the person authenticates at the AS and creates or binds an account. The AS then holds `sub` to account and issues without interaction for that person thereafter. This is the same one-time binding the AS already performs to establish trust with a person server, doing double duty: the question "which account does this person server's `sub` correspond to" is answered at the moment the AS is already asking the person who they are.

Because `sub` is directed per person server, a person reaching the same resource through two person servers presents two identifiers. The binding interaction is what lets the resource attach both to one account, and a resource that skips it sees two people and bills two ledgers.
...
| `budget_consumed` | Recent per-token consumption records reported by a resource | IETF | This document, (#budget-consumed) |
...
- draft-hardt-aauth-budgets-02
  ...
  - Added The Billing Account (#billing-account): which account a metered resource charges, and the base-protocol mechanisms that answer it — `(iss, sub)` lookup, the `account` parameter and claim, resource-initiated interaction for a person with no account yet, and the AS's own binding interaction in four-party access. No new mechanism; the document was silent on a question every metered resource meets on its first request.
