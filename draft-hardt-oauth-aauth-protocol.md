...
- **Person Server** decides whether to issue an auth token for a given resource and scope — based on user consent and, when the agent is operating under a mission, the mission's intent and prior log entries against the PS's supervision policy. This document defines the artifacts that carry the outcome — an auth token, a denial, `requirement=clarification` to the agent, `requirement=interaction` to the person — and not the policy. How a PS reaches allow, deny, clarify, or escalate, and how it delegates that evaluation to a supervisor that may be a person or an agent, is the subject of a companion specification on AAuth supervision.
...
**Agent identity access and above:**
...
**PS authorization access (three-party) and above:**
