...
A resource MUST have verified a person token (#person-tokens) or an auth token (#auth-tokens) on the request before it issues a resource token (#resource-token), and MUST challenge with `requirement=person-token` (#requirement-person-token) when it has verified neither. Only a person server can act on a resource token — in three-party it is the audience, and in four-party it is the only party that may call the AS token endpoint — so a resource token issued to an agent that cannot name a person is one nobody can redeem.
...
Clock skew is therefore the agent's to absorb, and the agent is the right party: it verifies nothing, and it is the only party that holds every token in the chain. An agent SHOULD refresh an agent, person, or auth token when fewer than five minutes remain before its `exp`, and SHOULD NOT present one inside that margin. The margin does not apply to a resource token: its lifetime is at most five minutes (#resource-token-structure), the agent redeems it at once, and no later party names it. Three things make the margin worth its cost:
...
1. Perform Auth Token Verification (#auth-token-verification) on the upstream token, with three substitutions: `aud` MUST equal the intermediary's identifier rather than the verifier's own (step 5); `cnf.jwk` is the calling agent's key and is not compared with the key that signed this request (step 6), which is the intermediary's; and the resource's record check on `sub` (step 7) does not apply. A token that fails is rejected with `invalid_upstream_token`, `expired_upstream_token` when only `exp` fails, or `revoked_upstream_token` when the recipient holds a revocation for it (#token-revocation).
...
- draft-hardt-oauth-aauth-protocol-11
  - Review fixes from the .NET SDK migration (2026-09-11). Call chaining: the upstream token's `cnf` is the calling agent's key, Upstream Token Verification states its substitutions to Auth Token Verification and that an intermediary is its own agent provider, and the token endpoint gains `invalid_`/`expired_`/`revoked_upstream_token`. Person token endpoint: how the PS resolves the person and mission from an upstream token. `updated_request` carries `presented_token` (REQUIRED) and fixes `mission_s256` and `tenant`; the same body serves PS-to-AS. A resource token follows a verified person token or auth token (person token on the authorization endpoint). Pending URL `410` yields to the retained-result rules, keyed by auth token `jti`. A revoked auth token is answered with `requirement=person-token`, not a resource token that would name it. The refresh margin excludes resource tokens. The `exp` minus `iat` check is MUST only where the ceiling is. A claims response carries no `sub`. Supervision and mission control companions marked TBD. Person token exposure wording distinguishes the key holder from a third party.
