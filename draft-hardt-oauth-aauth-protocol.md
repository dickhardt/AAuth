...
- **Agent identity without pre-registration**: A domain, static metadata, and a JWKS establish an identity any party can verify — no portal, no bilateral agreement, no shared secret (#agent-identity).
- **Proof-of-possession on every request**: HTTP Message Signatures ([@!RFC9421]) bind every request to the agent's key. No credential in the protocol is a bearer credential; a stolen token is useless without the key (#http-message-signatures-profile).
- **Person identity that survives a change of agent**: A person token identifies the person the agent acts for, and the resource keys its account and history on `(iss, sub)` — so the person can change agents without arriving at the resource as a stranger (#person-tokens).
- **Governance over what agents do**: A mission states the agent's intent in natural language for the person to approve; permission, audit, and interaction relay run through the person server — governance over decisions that cannot be reduced to machine-evaluable rules (#agent-governance).
- **Cross-domain authorization without coordination**: The PS federates with any resource's AS on first contact; each party adopts independently, and the first API call can be the registration (#ps-as-federation, #incremental-adoption).
...
AAuth does not replace OAuth. OAuth remains the right protocol for applications with a fixed, pre-registered set of integrations, and works better for flows that originate on the web — a user in a browser authorizing a site they are visiting — and need none of the functionality AAuth adds. AAuth addresses the clients OAuth cannot serve: those that discover resources at runtime and cannot pre-register with each one (see (#why-not-extend-oauth)).
