...
- **Supervisor**: The party the PS consults for a per-act decision within a mission — the Person by default, or a supervision server (SS) the PS delegates to (#roles). Not visible on the wire.
...
One further role does not appear on the wire. The **Supervisor** is the party the PS consults for a per-act decision (#policy-evaluation-points): whether this token request, permission request, or mission update is consistent with the mission's intent and with what the agent has already done under it. The Person is the Supervisor by default — a consent screen is the Person supervising — and consent remains the Person's word for approving a mission, or an access, directly. A PS MAY delegate supervision to a **supervision server (SS)**: a service that represents the Supervisor to the PS as the PS represents the Person to everyone else, so that routine decisions are answered without waking the Person and the Person's standing policy is applied consistently. The protocol between a PS and an SS is AAuth Supervision, a companion specification. Nothing an agent, resource, or AS sees changes with who supervises.
...
- **Person Server** decides whether to issue an auth token for a given resource and scope — based on user consent and, when the agent is operating under a mission, the mission's intent and prior log entries against the PS's supervision policy. The party that evaluates it is the Supervisor (#roles); how a PS consults a supervision server is the subject of AAuth Supervision, a companion specification.
