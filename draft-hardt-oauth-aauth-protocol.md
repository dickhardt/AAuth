...
The agent generates a signing key pair, proves its identity to its agent provider through a platform-specific mechanism, and receives an agent token binding the key to its identifier (#agent-tokens). The agent token MAY carry a `ps` claim naming the agent's person server. Acquisition is platform-dependent; see [@?I-D.hardt-aauth-bootstrap] for common patterns.
...
- draft-hardt-oauth-aauth-protocol-11
  - Readability restructure. The Protocol Overview is an overview: it says how the parties fit together and points to the sections that define each part; the agent token comes first, the person-identity flow shows the person token being obtained and the three- and four-party flows start from presenting it, and the "orthogonal" framing of governance is gone, since missions reach a resource only in the modes where a person token is presented. The Introduction's "Relationship to Existing Standards" is "Built On", and the two paragraphs that belonged to "What AAuth Provides" moved there. The Terminology no longer repeats how each server role is identified. Agent Identity is Agents, with an Agent Provider subsection that collects what an AP does: issues agent tokens and sub-agent tokens, evaluates policy by reissuing or refusing them, and revokes them at the PS. Person Token is no longer its own section; its structure, usage, and verification sit under the Person Token Endpoint. Resource Access is ordered by what an agent encounters, from the `agent-token` requirement through resource-managed authorization and the session token, the `person-token` and `auth-token` requirements, the authorization endpoint, and the resource token. Person Server follows the agent's path: the person token endpoint, the auth token endpoint, user interaction, consent presentation, and clarification chat, then the interaction, permission, and audit endpoints, then re-authorization. Protocol Primitives is the normative reference and is stated without rationale, ordered from identifiers and metadata through signatures, key discovery, and the common JWT profile to requirement responses, capabilities, deferred responses, errors, scopes, account binding, and revocation; the `exp` and `iat` verification rules moved there from the refresh margin, and Token Revocation is split into request, who revokes what, response, cascade, and presenting a revoked token. Rationale that Protocol Primitives carried moved to the Design Rationale appendix. Each normative statement is made once and referenced elsewhere.
  ...
  - A resource token MAY carry `login_hint`, which the agent passes to its PS as the `login_hint` parameter of the token request. A resource stepping up for an identity claim about one particular person — `scope: email` for an address it already holds — knew who it was asking about and had nothing to carry the value in, so the agent had nothing to forward. It is a hint: the PS MAY ignore it, and a resource checks the claims it receives against what it asked for. Issue #163.
  - The Adoption Matrix names the auth token, not the person token, in the Agent column of the PS authorization and Federated authorization rows. In both modes the person token is only what the agent presents to have a resource token issued; the credential the resource verifies and enforces is the auth token, which is what the Agent column names in every other row. The Person identity row is unchanged, since there the person token is the credential. Issue #161.
  - The person token request takes the OPTIONAL parameters of the auth token request — `capabilities`, `login_hint`, `tenant`, `domain_hint`, `prompt`, `justification`, `platform`, and `device` — and the `AAuth-Capabilities` cross-reference names both PS endpoints rather than only the auth token endpoint. The endpoint listed four parameters and no way to declare a capability, name a tenant, or pass a hint, while the same section lets the PS answer `202` with `requirement=interaction` and defines `user_unreachable` as terminal when the agent did not declare the `interaction` capability — which there was no conforming way to do. Two implementations diverged on it: an agent sent `AAuth-Capabilities`, which the spec rules out on PS endpoints, a PS read `capabilities` and `tenant` from the body as an extension, and before that a first-contact request was answered `user_unreachable` when the agent could in fact have driven the interaction. Issues #175, #177.
  - Call chaining accepts a person token as `upstream_token`, at the person token endpoint, the auth token endpoint, and the PS-to-AS request. The upstream token is a token the calling agent presented on a request the intermediary served, so an intermediary that serves on identity alone can chain; before, only an auth token was defined and such an intermediary had nothing to present. The PS used nothing an auth token adds over a person token: it resolves the person from `aud` and `sub`, reads `mission_s256`, and does not require downstream access to be a subset of upstream scope. Routing follows a person token's `iss` or an auth token's `ps`. Upstream Token Verification branches on `typ`, and the revocation cascade walks from a person token presented upstream as well. The rule rejecting a person token where an auth token is required does not apply to `upstream_token`. An intermediary MAY reuse an upstream token until it expires, and downstream access does not outlive it: a person token issued with `upstream_token`, and an auth token issued on a request carrying one, expire no later than the upstream token; a pending downstream request whose upstream token expires ends with `expired`. The PS copies the upstream token's `mission_s256` into the downstream person token, so the mission reaches the chain; before, the PS evaluated against it but no downstream token carried it. The PS verifies an upstream auth token's `iss` is itself or an AS it presented a person token to for that `aud` and `sub`; the AS checks only that the upstream token names the PS that signed the request. The PS rejects a chain whose calling agent's agent token or person binding it has revoked, with `revoked_upstream_token`. A sub-agent's agent token `iss` MUST equal its parent's, checked with `invalid_subagent_token`.
  - Intermediary Agent Identity: an intermediary MUST be its own agent provider, with `aauth-agent.json` on its own origin, `issuer` equal to its resource `issuer`, and a self-issued agent token. This was stated only in the explanation of an upstream token verification step. It is what lets the recipient compare the upstream token's `aud` with the signer: the agent token's `iss` is the only resource-scoped identifier the request carries. Upstream Token Verification names where the intermediary's agent token is found: `Signature-Key` at the PS, the `agent_token` parameter at the AS. A request carrying `upstream_token` neither uses nor establishes an agent-person binding. Roles under a shared origin share one identifier, distinguished by `dwk`.
  - A revocation recipient answers once its cascade is terminal. A PS receiving an agent token revocation and an AS receiving a person token revocation record the revocation first, then revoke downstream what they issued from it, and return `200` only when every downstream revocation has a terminal outcome, returning a `202` deferred response when that takes longer than they will hold the connection or a downstream party itself answered `202`. The `200` said only that the recipient had recorded the revocation, so the initiator could not tell whether the tokens issued from the revoked one were gone. A downstream revocation ends recorded, `revocation_unsupported` (no `revocation_endpoint`, or `unsupported_iss`; the party honors the tokens until `exp`), or `revocation_unavailable` (no response, timeout, `5xx`, malformed; retryable). An AS reports the outcome to the PS in a `downstream` array, one entry per resource, which discloses nothing the PS did not know; a PS reports nothing to the AP, since even a count would say which resources the person uses through that agent. Revocation is idempotent: a repeated revocation re-attempts what did not succeed and reports the current outcome. The AS's cascade to the resource is now a MUST, since the PS reads how far a revocation went from it. A revocation of a person token now also reaches a chain: the PS records the upstream token's `(iss, jti)` on each person token it issues with `upstream_token`, and SHOULD revoke those person tokens when the earlier person token the upstream token was issued against is revoked. Every hop routes to the same PS, so the link is in its own records; an AS records nothing of the chain, since it issues against the presented person token as in any other request. Without it an intermediary kept access downstream to a person who had withdrawn the upstream grant, for up to an hour. Addresses issue #173.
  ...
  - `presented_jti` names the token the request carried — the person token on a first challenge, the auth token on a step-up or per-call challenge — and the agent passes that token to the PS as `presented_token` (REQUIRED), which the PS passes on to the AS. PS and AS run one verification: signature against the issuer, `aud` the resource, `cnf.jwk` against `agent_jkt`, `jti` against `presented_jti`, and `sub`, `mission_s256`, and `tenant` against the resource token, which makes mission stripping detectable. An auth token's `exp` is capped at the presented token's. `updated_request` carries `presented_token`. Errors: `invalid_presented_token`, `expired_presented_token`, `revoked_presented_token`. Issues #95, #152.
  - Missions: replaced the `mission` object with `mission_s256` in person, resource, and auth tokens; removed the `AAuth-Mission` header; the approval response carries the blob base64url-encoded with `s256` beside it, `capabilities` outside the blob, and a person token for each of the `resources` the proposal named. The blob gained `approved_resources`, MAY carry `expires_at`, and lost `approver`: the PS is always the approver, and the mission is identified by the PS and `s256`. Member lists are a floor. The mission endpoint is the owning agent's surface with three operations of one shape — propose, `action: update`, `action: completion`; an update appends to the log without changing the blob; completion moved off the interaction endpoint. Two states, active and terminated, an open set of termination reasons, and one error, `mission_terminated`, carrying `termination_reason`. Mission endpoint errors are indistinguishable for a missing mission and one the agent does not own (adopted from draft-mcguinness-mission-aauth-management). `mission_control_endpoint` is the control plane for other principals; its specification is TBD.
  - Call chaining routes on the auth token's `ps` claim. The intermediary obtains a person token downstream with `upstream_token`, whose `cnf` is the calling agent's key; Upstream Token Verification states its substitutions; errors `invalid_upstream_token`, `expired_upstream_token`, `revoked_upstream_token`.
  - Token Revocation reworked: the request is `jti` and `exp`; a caller revokes only its own tokens and the recipient keys the entry under the verified caller; agent tokens are revoked only at a PS, person and auth tokens at the resource, resource tokens at the party in `aud`; the four-party cascade runs through the person token; `200 OK` whether or not the recipient holds the token; `revocation_endpoint` is RECOMMENDED for a PS, an AS, and a resource accepting person tokens. A revoked token in the `Signature-Key` header is answered `revoked_jwt`, with `requirement=person-token` beside it for a revoked auth token; a revoked parameter token is answered `revoked_<parameter>_token`; a pending request terminates with `revoked`. Issues #146, #154.
  - Expiry and the Refresh Margin: `exp` is judged by the verifier's clock with no tolerance; `iat` is REQUIRED and not a validity check, with an OPTIONAL bound answered `clock_skew`; the agent refreshes agent, person, and auth tokens with five minutes left, from the top of the chain; `exp` minus `iat` is checked against the one-hour ceiling of person and auth tokens.
  - Server-to-server signing pinned: a PS, AS, AP, or resource signing in its own right uses the `jwks_uri` scheme with `id` its metadata `issuer` and `dwk` that document's name; a resource acting as an agent signs under the `jwt` scheme. Requests with a body to a PS or AS endpoint MUST cover `content-digest` and `content-type`. Added the OPTIONAL common metadata field `accept_signature_algs` (issue #94). A verifier that first sees an artifact after a delay uses the signed `created`.
  - Consent Presentation: resource-asserted and agent-asserted content MUST be visually distinguished and attributed, and a PS MUST NOT decide on agent-asserted content alone; Security Considerations gained Agent Control of the Consent Surface. Section structure for the Markdown-valued parameters is deferred to one later definition (editor's note in Terminology).
  - Named the Supervisor — the Person, or a supervision server the PS delegates to under a companion specification that is TBD — and distinguished supervision from governance. Stated the PS conformance floor (four REQUIRED metadata fields) and added the informative appendix A Minimal Person Server. Removed Third-Party Login and `login_endpoint` (issue #155).
  - `requirement=auth-token` MAY be delivered as a `202` deferred response holding the invocation; completion answers a repeated presentation of the same auth token from the retained result, keyed by `jti` (issue #92). The host of an interaction URL MAY complete the interaction over its own channel. Added `as_unreachable` (502) and relay of an AS's terminal error. Added the `aauth-resource` link relation. Corrected the JWT Claims Registrations table: `presented_jti`, `account`, and `interaction` added, `agent` removed, `ps` once.
  - Consistency pass across the family (2026-09-13): common JWT claims and verification stated once (#aauth-tokens) and referenced from each token; typed error codes only for tokens carried as parameters, the `Signature-Key` token failing generically (`invalid_person_token`, `invalid_agent_token`, and `expired_agent_token` removed; `subagent_token` codes added); missing signature headers are `invalid_signature`; every party MUST support `Ed25519`; `scope` in a resource token is OPTIONAL where a companion claim replaces it; the localhost callback exception stated; `Accept-Signature-Scheme` in place of the removed `sigkey` parameter; RFC 9651 in place of RFC 8941; the Payment scheme named by its draft's title; prose says "the `jwt` scheme" and diagrams say HTTP Sig; example JWT placeholders decode to `Ed25519` and current `typ` values; the person-token prerequisite, refresh guidance, step-up, routing, same-origin, and PS-floor statements reduced to one normative statement each; `date` removed from the front matter so the build stamps it.
