...
- `cnf`: `jwk` is the agent's public key
- `exp`: Person tokens MUST NOT have a lifetime exceeding 1 hour, and MUST NOT outlive the agent token presented when the token was requested or, when `mission_s256` is present, the mission's `expires_at` (#mission-approval).
...
2. Verify the issuer. A person token's `iss` MUST be the PS handling the chain: the PS itself, or at the AS the PS that signed the request. An auth token's `ps` MUST name that PS, and its `iss` MUST be that PS or an AS the PS federated with to obtain it.
...
4. The PS evaluates the request against the mission and its supervision policy, based on the upstream token's claims and mission context. The resulting downstream authorization is not required to be a subset of any upstream authorization — see (#call-chaining).
...
When a resource needs to access a downstream resource on behalf of the caller, it acts as an agent — the intermediary. The upstream token is the token the calling agent presented in the `Signature-Key` header of the request the intermediary is fulfilling: a person token when the intermediary served that request on the person's identity (#overview-person-identity), an auth token when it required authorization. An intermediary MUST NOT present any other token as the upstream token. A resource that requires an auth token does not serve a request carrying only a person token (#person-token-not-authorization), so it has no such request to fulfill.
...
| `revoked_upstream_token` | 400 | The upstream token has been revoked by the server that issued it (#token-revocation). Terminal for that token. |
| `invalid_subagent_token` | 400 | Sub-agent token malformed, signature verification failed, or its `parent_agent` does not name the signing agent (#sub-agents) |
...
- draft-hardt-oauth-aauth-protocol-11
  - Call chaining accepts a person token as `upstream_token`, at the person token endpoint, the auth token endpoint, and the PS-to-AS request. The upstream token is the token the calling agent presented to the intermediary on the request it is fulfilling, so an intermediary that serves on identity alone can chain; before, only an auth token was defined and such an intermediary had nothing to present. The PS used nothing an auth token adds over a person token: it resolves the person from `aud` and `sub`, reads `mission_s256`, and does not require downstream access to be a subset of upstream scope. Routing follows a person token's `iss` or an auth token's `ps`. Upstream Token Verification branches on `typ`, and the revocation cascade walks from a person token presented upstream as well. The rule rejecting a person token where an auth token is required does not apply to `upstream_token`.
