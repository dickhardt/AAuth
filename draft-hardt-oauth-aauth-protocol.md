...
- draft-hardt-oauth-aauth-protocol-10
  - Algorithm identifiers: adopted the fully-specified `Ed25519` of [@!RFC9864] in place of the polymorphic `EdDSA` it deprecates. Rewrote (#signature-algorithms) to state the Algorithm Determination rules inherited from [@!I-D.hardt-httpbis-signature-key] — `alg` REQUIRED and fully specified, `EdDSA`, `none`, and symmetric algorithms prohibited, and a key whose `kty` or `crv` disagrees with its `alg` rejected — and cited the IANA JOSE registry rather than [@RFC7518]. Addresses issue #57.
  - Keys: a `cnf` JWK MUST carry a fully-specified `alg`, in agent and auth tokens alike, and so MUST every key at an AAuth server's `jwks_uri`, which has no other channel for it; examples updated. A verifier selects the key matching `kid` without requiring the other JWKS members to be usable, so an issuer may publish a post-quantum key beside a classical one.
  - Verification: reordered so the algorithm is read from the key the scheme resolved to, which under `scheme=jwt` is not held until the assertion verifies. Added a scheme-rejection step returning `unsupported_scheme` with `Accept-Signature-Scheme`; replaced "determine the algorithm from the `alg` parameter" with specific checks reporting `unsupported_algorithm` with `Accept-Signature-Alg`, and `invalid_key` on a `kty` or `crv` mismatch.
  - Errors: mapped the metadata issuer check to `issuer_missing` and `issuer_mismatch`, and required `issuer` to be present rather than only to match. Stated that this profile pins signature failures to `401` where the base specification uses `400` for most of them, and that a `403` MUST NOT carry `Signature-Error` or either `Accept-Signature-*` header.
  - Forbade the `alg` signature parameter and discouraged `keyid`, per [@!RFC9421], Sections 3.3.7 and 5.1: the algorithm is signaled by the key, and the key by `Signature-Key`.
  - Rewrote the `requirement=agent-token` comparison, which described the `sigkey` parameter of `Accept-Signature`, removed in draft-hardt-httpbis-signature-key-08.
  - Revocation identifies a token by `(iss, jti)` rather than `jti` alone, which is unique only within its issuer's namespace; recipients key revocation state by the pair. Addresses issue #59.
  - An agent provider revokes an agent token at the PS's `revocation_endpoint`, found from the `ps` claim it placed in the token; the PS then denies that token and SHOULD revoke what it issued or provided for that agent. Identity-based access does not pass through the PS and is not reachable this way. Addresses issue #60.
  - Stated what revocation does not do: verification fetches the issuer's JWKS for a key but never asks the issuer about the token, so a holder no revocation request reaches is bounded by token lifetime alone.
  - Added (#directed-sub-chaining): a downstream issuer MUST NOT copy a directed `sub` from an upstream token, MAY emit one only from its own authenticated federation step, and never places a person identifier in `act`. Identity is `(iss, sub)`, so a value minted for one audience means nothing under another. Updated the delegation chain examples, which carried a single `sub` unchanged across every hop. Addresses issue #41.
  - Added (#account-binding): an OPTIONAL `account` parameter on the authorization endpoint request, echoed in the resource token and copied into the auth token, binding an authorization to one of several accounts a resource may hold for the same person. The value is opaque, in the resource's own namespace, and selects an account rather than hinting at one, which is why it is not `login_hint`. Addresses issue #52.
  ...
  - Referenced HTTP Signature Keys and AAuth Bootstrap by datatracker document URL, which tracks the latest revision, rather than a pinned draft revision. Drafts not yet on the datatracker (R3, Events) continue to reference the repository.
  - Acknowledgments: named He Gu, previously listed by the handle hegu-1, and removed sdatapix, a duplicate of Sanjay Dalal.
