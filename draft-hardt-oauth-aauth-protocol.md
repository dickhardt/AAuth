...
2. Verify the issuer. At the PS: a person token's `iss` MUST be this PS; an auth token's `ps` MUST be this PS, and its `iss` MUST be this PS or an AS this PS obtained an auth token from. At the AS: a person token's `iss`, or an auth token's `ps`, MUST be the PS that signed the request. The AS does not check an auth token's `iss` beyond verifying its signature; the PS has already done so.
