...
A verifier judges `exp` against its own clock. This document defines no tolerance for clock skew on `exp`: a token is expired the moment the verifier's clock passes it, at every party that verifies it. `iat` records when the issuer signed and is not a validity check; a verifier MAY use it for audit, or to bound the age of a token by its own policy, but MUST NOT reject a token because `iat` is later than its own clock.
