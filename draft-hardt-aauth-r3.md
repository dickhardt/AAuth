...
The R3 hash (`r3_s256`) is computed as the SHA-256 hash of the bytes of the R3 document as served by the resource at `r3_uri`, base64url-encoded without padding. There is no canonicalization step: the hash covers the exact response body the resource returns, and verifiers hash the bytes they receive.

The resource MUST serialize the R3 document once, persist those exact bytes, and serve them verbatim on every request for the same `r3_uri`. Re-serializing the document between hashing and serving will break verification. Common breakage: middleware that re-stringifies JSON, CDN minification, and framework helpers that reorder keys. Resources that build R3 documents on the fly MUST store the serialized bytes (e.g. in a key-value store) rather than re-building and re-serializing them per request.
...
3. Compute the SHA-256 hash of the bytes received when fetching the document and compare it to `r3_s256`. If the hashes do not match, the AS MUST reject the resource token.
